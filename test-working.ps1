# Simple Load Test - Final Working Version
Write-Host "🧪 SIMPLE LOAD TEST - SYSTEM VERIFICATION" -ForegroundColor Green
...
for ($batchNum = 0; $batchNum -lt $totalBatches; $batchNum++) {
    ...
    $batchDisplay = $batchNum + 1
    Write-Host "📤 Batch $batchDisplay/$totalBatches to Producer $producerPort - $currentBatchSize messages" -ForegroundColor Cyan
    ...
    for ($i = 1; $i -le $currentBatchSize; $i++) {
        ...
        $messageText = "Quick test message $messageId from Producer $producerPort at $(Get-Date -Format 'HH:mm:ss.fff')"
